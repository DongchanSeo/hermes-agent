...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    # gateway flaps. The CMD process is a no-op heartbeat that keeps
    # /init alive until `docker stop` sends SIGTERM, at which point
    # /init runs stage 3 shutdown (which tears down the supervised
    # gateway cleanly).
    #
    # We block in-process with signal.pause() rather than
    # os.execvp("sleep", ...).  The exec form did a PATH lookup for the
    # `sleep` binary and crashed the whole container with
    ...
    # images without a `sleep` on PATH (issue #36208).  Blocking on a
    # signal needs no external binary, so it can't fail that way.
    _block_until_terminated()
    return True  # unreachable; _block_until_terminated() blocks forever


def _block_until_terminated() -> None:
    """Keep the s6 CMD process alive until the container is stopped.

    Installs a SIGTERM handler that exits with the conventional
    128+signum code so `docker stop` produces a clean, expected exit,
    then blocks on signal.pause().  Falls back to threading.Event().wait()
    on platforms without signal.pause() (e.g. Windows) — although this
